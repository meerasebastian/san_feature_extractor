/*Name   : san_feature_extractor
 Author  : Meera Sebastian 
*/
#include <ros/ros.h>
...
int personFound = 0, hallwayFound = 0, testflag = 0;
float personInitialX = 0, personInitialY = 0; 
float hallwaySlope, hallwayIntercept, hallwayWidth;
...
void transformPoint(const tf::TransformListener& listener){
  ...
  hallwayPoints1.header.stamp = ros::Time::now();
  ...
  hallwayPoints2.header.stamp = ros::Time::now();
  ...
  try{
    ...
    //Not required: Person and robot positions are now based on world frame
...
void featureCalculator(float robotPositionX, float robotPositionY){
  ...
  if( personFound == 0 ){
    ...
    personHallwayL = ( fabs( hallwaySlope*personData.xdistance - personData.ydistance + hallwayIntercept ) ) / ( sqrt( pow(hallwaySlope,2) + 1 ) );
    personHallwayR = hallwayWidth - personHallwayR;
    robotHallwayL = ( fabs( hallwaySlope*robotPositionX - robotPositionY + hallwayIntercept ) ) / ( sqrt( pow(hallwaySlope,2) + 1 ) );
    robotHallwayR = hallwayWidth - robotHallwayR;
    ...
    ROS_INFO("Transformed slope & intercept : %f & %f",hallwaySlope, hallwayIntercept);
    ROS_INFO("Hallway width : %f", hallwayWidth);
    ...
    //ROS_INFO("Time - %f - %s",normalized_time,stringData.c_str());
...
//For Real world scenario - Person Detection
...
void hallwayDetectionCallback(const hallway::hallwayMsg::ConstPtr& msg){
	//For testing service -  SAN_NODES
  	...
  	hallwayWidth = msg->width_hallway; //Width of the Hallway
  	...
  	hallwaySlope = (float)(transformedHallwayPoints2.point.y   - transformedHallwayPoints1.point.y)/(float)(transformedHallwayPoints2.point.x - transformedHallwayPoints1.point.x);
  	hallwayIntercept = ( transformedHallwayPoints1.point.y  -  hallwaySlope*transformedHallwayPoints1.point.x ) * 100;  
